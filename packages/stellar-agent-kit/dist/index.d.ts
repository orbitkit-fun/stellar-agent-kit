...
declare function getNetworkConfig(name?: NetworkName): NetworkConfig;
...
interface LendingWithdrawArgs {
    poolId: string;
    assetContractId: string;
    amount: string;
}
interface LendingRepayArgs {
    poolId: string;
    assetContractId: string;
    amount: string;
}
 ...
 */
declare function lendingWithdraw(networkConfig: NetworkConfig, secretKey: string, args: LendingWithdrawArgs): Promise<LendingResult>;
/**
 * Repay a borrowed asset to a Blend pool.
 */
declare function lendingRepay(networkConfig: NetworkConfig, secretKey: string, args: LendingRepayArgs): Promise<LendingResult>;
...
declare class StellarAgentKit {
     ...
     * @returns Transaction hash
     */
    createTrustline(assetCode: string, assetIssuer: string, limit?: string): Promise<{
        hash: string;
    }>;
    /**
     ...
     * @returns Transaction hash
     */
    removeTrustline(assetCode: string, assetIssuer: string): Promise<{
        hash: string;
    }>;
    /**
     ...
     */
    lendingWithdraw(args: LendingWithdrawArgs): Promise<LendingResult>;
    /**
     * Repay a borrowed asset to a Blend pool.
     */
    lendingRepay(args: LendingRepayArgs): Promise<LendingResult>;
}
...
declare const StellarEnvSchema: z.ZodObject<{
...
declare const X402EnvSchema: z.ZodObject<{
    X402_DESTINATION: z.ZodString;
}, "strip", z.ZodTypeAny, {
    X402_DESTINATION: string;
}, {
    X402_DESTINATION: string;
}>;
declare const SoroSwapEnvSchema: z.ZodObject<{
    SOROSWAP_API_KEY: z.ZodString;
}, "strip", z.ZodTypeAny, {
    SOROSWAP_API_KEY: string;
}, {
    SOROSWAP_API_KEY: string;
}>;
declare const McpEnvSchema: z.ZodObject<{
    SECRET_KEY: z.ZodString;
    SOROSWAP_API_KEY: z.ZodString;
}, "strip", z.ZodTypeAny, {
    SOROSWAP_API_KEY: string;
...
type X402Env = z.infer<typeof X402EnvSchema>;
type McpEnv = z.infer<typeof McpEnvSchema>;
/**
 ...
 */
declare function validateStellarEnv(env?: NodeJS.ProcessEnv): StellarEnv;
/**
 * Validate required env vars for x402-stellar-sdk server middleware.
 */
declare function validateX402Env(env?: NodeJS.ProcessEnv): X402Env;
/**
 ...
 */
declare function validateMcpEnv(env?: NodeJS.ProcessEnv): {
    success: true;
    data: McpEnv;
} | {
    success: false;
...
export { ALLBRIDGE_CORE_STELLAR_DOCS, BAND_ORACLE, BLEND_POOLS, BLEND_POOLS_MAINNET, type DexAsset, type DexClient, FXDAO_MAINNET, type LendingBorrowArgs, type LendingRepayArgs, type LendingResult, type LendingSupplyArgs, type LendingWithdrawArgs, MAINNET_ASSETS, McpEnvSchema, type NetworkConfig, type NetworkName, type OracleAsset, type PriceData, type QuoteResult, REFLECTOR_ORACLE, type ReflectorOracle, type ReflectorOracleConfig, SOROSWAP_AGGREGATOR, SoroSwapEnvSchema, StellarAgentKit, type StellarAsset, StellarEnvSchema, type StellarNetwork, type SwapResult, TESTNET_ASSETS, X402EnvSchema, createDexClient, createReflectorOracle, getNetworkConfig, lendingBorrow, lendingRepay, lendingSupply, lendingWithdraw, networks, validateMcpEnv, validateStellarEnv, validateX402Env };
