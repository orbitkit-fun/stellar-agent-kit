...
const decimals =
upper === "XLM" ? 7 : upper === "USDC" || upper === "AUSDC" ? 6 : 7;
...
function validatePositiveAmount(amount: string, fieldName: string = "amount"): void {
const normalized = amount.trim().replace(/\s*(XLM|USDC|AUSDC)$/i, "");
...
```
  ...
  if (privateKey && (privateKey.length !== 56 || !privateKey.startsWith("S"))) {
    throw new Error(
      `Invalid private key. Must be exactly 56 characters starting with S. Got ${
        privateKey?.length || 0
      } characters. Please provide the complete private key.`
    );
  }
...
name: "send_payment",
description:
"Send XLM or a custom asset (e.g. USDC) from your account to a destination address.",
parameters: z.object({
...
.string()
.optional()
.describe("Issuer address (G...) for custom assets; required when assetCode is set"),
network: z.literal("mainnet").optional().default("mainnet"),
}),
execute: async ({
privateKey,
...
```
  ...
  try {
    ...
    const expectedInHuman = (
      parseInt(quote.expectedIn) / Math.pow(10, fromDecimals)
    ).toFixed(fromDecimals);

    const expectedOutHuman = (
      parseInt(quote.expectedOut) / Math.pow(10, toDecimals)
    ).toFixed(toDecimals);
    ...
    return {
      ...
      message: `Quote: ${expectedInHuman} ${fromAsset
        .trim()
        .toUpperCase()} → ${expectedOutHuman} ${toAsset.trim().toUpperCase()}`,
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
